...
class FileChunk : public virtual ::apache::thrift::TBase {
 public:
  ...
  FileChunk() : fileName(), data(), isExecutable(0), shippingCounter(0), fileFormat(), hash() {
  }
  ...
  void __set_shippingCounter(const int64_t val);
  ...
  bool operator == (const FileChunk & rhs) const
  {
    ...
    if (!(isExecutable == rhs.isExecutable))
      return false;
    if (!(shippingCounter == rhs.shippingCounter))
      return false;
...
class Task : public virtual ::apache::thrift::TBase {
 public:
  ...
  Task() : taskName(), taskDescription(), cmdToExec(), otherData(), estimatedLeadTime(0) {
  }
  ...
  void __set_cmdToExec(const std::string& val);
